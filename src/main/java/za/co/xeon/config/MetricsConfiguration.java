	...
	public static class SparkRegistry {
		...
		private void init() {
			if (jHipsterProperties.getMetrics().getSpark().isEnabled()) {
				...
				SparkReporter sparkReporter = SparkReporter.forRegistry(metricRegistry).convertRatesTo(TimeUnit.SECONDS).convertDurationsTo(TimeUnit.MILLISECONDS).build(sparkHost, sparkPort);
