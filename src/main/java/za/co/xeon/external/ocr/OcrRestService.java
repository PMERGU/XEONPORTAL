...
public class OcrRestService {
	...
	// public Task scanDocument(@RequestParam("podDocument") MultipartFile
	// podDocument) {
	// try {
	// // Get name of uploaded file.
	// String originalFileName = podDocument.getOriginalFilename().substring(0,
	// podDocument.getOriginalFilename().indexOf("."));;
	// String originalExtension =
	// podDocument.getOriginalFilename().substring(podDocument.getOriginalFilename().indexOf("."));
	// log.debug(originalFileName + "." + originalExtension);
	//
	// File tmpFile = Converters.multipartToFile(new
	// File(mobileConf.getPodDirectory()), originalFileName, originalExtension,
	// podDocument);
