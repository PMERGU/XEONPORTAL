...
public class AttachmentService {
    ...
    @Autowired
    public AttachmentService(UserRepository userRepository,
    ...
    public ReadAttachmentDto readAttachment(final String uuid, final Boolean activated) {
        ...
        return attachmentRepository.findByUuidAndActivated(uuid, activated)
                .map(attachment -> {
                    ReadAttachmentDto readAttachment = null;
                    try {
                        readAttachment = new ReadAttachmentDto(attachment.getMimeType(),
                                s3Service.retrieveFile(attachment.getFileName()));
    ...
    public void deleteAttachment(final String uuid) {
        attachmentRepository.findByUuidAndActivated(uuid)
                .ifPresent(attachment -> attachment.setActivated(false));
    }
