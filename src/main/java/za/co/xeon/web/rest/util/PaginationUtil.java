...
public class PaginationUtil {
	...
	public static HttpHeaders generateHttpHeaders(List list, String baseUrl) throws URISyntaxException {

		HttpHeaders headers = new HttpHeaders();
		headers.add("X-Total-Count", "" + list.size());
		String link = "";
		// if ((page.getNumber() + 1) < page.getTotalPages()) {
		// link = "<" + (new URI(baseUrl +"?page=" + (page.getNumber() + 1) +
		// "&size=" + page.getSize())).toString() + ">; rel=\"next\",";
		// }
		// prev link
		// if ((page.getNumber()) > 0) {
		// link += "<" + (new URI(baseUrl +"?page=" + (page.getNumber() - 1) +
		// "&size=" + page.getSize())).toString() + ">; rel=\"prev\",";
		// }
		// last and first link
		int lastPage = 1;
